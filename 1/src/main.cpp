...
int main(int argc, char** argv) {
...

    {
        ...
        MEASURE(iteration, cpp::rgb_to_bgr, src, dst, width, height);
    }

    {
        const auto dst = dst_neon.data();
        MEASURE(iteration, neon::rgb_to_bgr, src, dst, width, height);
...

    {
        ...
        MEASURE(iteration, cuda::rgb_to_bgr, d_src.get(), d_dst.get(), width, height);
