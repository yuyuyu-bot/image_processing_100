...
int main(int argc, char** argv) {
...

    {
        ...
        MEASURE(iteration, cpp::dilation, src, dst, width, height, ksize);
    }

    {
        const auto dst = dst_neon.data();
        MEASURE(iteration, neon::dilation, src, dst, width, height, ksize);
...

    {
        ...
        MEASURE(iteration, cuda::dilation, d_src.get(), d_dst.get(), width, height, ksize);
